...
class Net(nn.Module):
    def __init__(self, n_class, condition = False):
        ...
        self.conv5 = nn.Sequential(MaskedConv2d('B', fm, fm, 3, 1, 1, bias=False), nn.BatchNorm2d(fm), nn.ReLU(True),)
    ...
    def forward(self, x, h=None):
        if h is not None:
            ...
            return self.conv9(self.conv5(self.conv4(self.conv3(self.conv2(self.conv1(x))))))
...
outputdir = './gen3_{0}_{1}_{2}_{3}{4}'.format(config['dataset'], args.first_unseen, threshold, args.cc_max, args.suffix)
