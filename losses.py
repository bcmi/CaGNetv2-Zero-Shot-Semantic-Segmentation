...
class ClsLoss(nn.Module):
    def __init__(self, opt):
        ...
        self.ps = opt['patchsize']
        assert (self.ps > 0 and self.ps % 2 == 1)
        self.criterionCLS = nn.CrossEntropyLoss(ignore_index=opt['ignore_index'])

    def forward(self, net, feat, gt, ignore_mask, select=False):
        cls_score = net(feat, 'cls')
        if select == True and self.ps != 1: # patch-wise finetuning
            H, W = gt.shape[1], gt.shape[2]
            assert (H % self.ps == 0 and W % self.ps == 0)
            selected_indices_H = torch.LongTensor(range((self.ps-1)//2, H, self.ps)).cuda()
            selected_indices_W = torch.LongTensor(range((self.ps-1)//2, W, self.ps)).cuda()
            cls_score = cls_score[:,:,:,selected_indices_W]
            cls_score = cls_score[:,:,selected_indices_H,:]
            gt = gt[:,:,selected_indices_W]
            gt = gt[:,selected_indices_H,:]
            ignore_mask = ignore_mask[:,:,selected_indices_W]
            ignore_mask = ignore_mask[:,selected_indices_H,:]
