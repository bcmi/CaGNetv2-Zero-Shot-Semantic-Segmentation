...
def parse_args():
    ...
    parser.add_argument('--ep', type=int, default=10, help='epoch total')
...
if not os.path.exists('./blocks_new_v3'):
    os.mkdir('./blocks_new_v3')
logger = logWritter('blocks_new_v3/log_{0}.txt'.format(config['dataset']))
...
for epoch in range(args.resume_from, args.ep):
    ...
    # save models
    model_name = os.path.join('blocks_new_v3', '{0}_{1:0>3d}.pth'.format(config['dataset'], epoch + 1))
    torch.save(
            {
                'epoch': epoch + 1,
