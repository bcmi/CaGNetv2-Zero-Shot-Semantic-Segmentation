...
def parse_args():
    ...
    parser.add_argument('--first_unseen', type=int, default=5, help='first several pixels are pre-set as unseen categories')
    parser.add_argument('--threshold', type=int, default=7, help='at least threshold pixels in a block should be unseen')
    parser.add_argument('--cc_max', type=int, default=3, help='maximum category count within each block')
...
class Net(nn.Module):
    def __init__(self, n_class, condition = False):
        ...
        d = 600
        self.conv1 = nn.Sequential(MaskedConv2d('A', d,  fm, 7, 1, 3, bias=False), nn.BatchNorm2d(fm), nn.ReLU(True))
        self.conv2 = nn.Sequential(MaskedConv2d('B', fm, fm, 7, 1, 3, bias=False), nn.BatchNorm2d(fm), nn.ReLU(True),)
        self.conv3 = nn.Sequential(MaskedConv2d('B', fm, fm, 7, 1, 3, bias=False), nn.BatchNorm2d(fm), nn.ReLU(True),)
        self.conv4 = nn.Sequential(MaskedConv2d('B', fm, fm, 7, 1, 3, bias=False), nn.BatchNorm2d(fm), nn.ReLU(True),)
    ...
    def forward(self, x, h=None):
        if h is not None:
            ...
            x = self.conv3(x) + torch.matmul(h,self.b3).permute(0,3,1,2).contiguous()#.view(b,c,1,1)
            x = self.conv4(x) + torch.matmul(h,self.b4).permute(0,3,1,2).contiguous()#.view(b,c,1,1)
            ...
            x = self.conv7(x) + torch.matmul(h,self.b7).permute(0,3,1,2).contiguous()#.view(b,c,1,1)
            x = self.conv8(x) + torch.matmul(h,self.b8).permute(0,3,1,2).contiguous()#.view(b,c,1,1)
            return self.conv9(x)
        else:
            return self.conv9(self.conv8(self.conv7(self.conv6(self.conv5(self.conv4(self.conv3(self.conv2(self.conv1(x)))))))))
