...
logger = logWritter('blocks_new_7/log_{0}.txt'.format(config['dataset']))
...
for epoch in range(args.resume_from, 10):
    ...
    for _, gt in tr:
        ...
        if iteration % display_iter == 0:
            ...
            model_name = os.path.join('blocks_new_7', 'example.pth')
            torch.save(
                    {
                        'epoch': 0,
    ...
    # save models
    model_name = os.path.join('blocks_new_7', '{0}_{1:0>3d}.pth'.format(config['dataset'], epoch + 1))
    torch.save(
            {
                'epoch': epoch + 1,
                'state_dict': net.state_dict()
