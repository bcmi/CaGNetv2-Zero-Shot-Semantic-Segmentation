...
def get_embedding(cfg):
    dataset_path = cfg['datadir'] + '/' + cfg['dataset']
...
def get_split(cfg):
    ...
    seen_classes = np.concatenate((np.load(dataset_path + '/split/seen_cls.npy'), np.load(dataset_path + '/split/val_cls.npy')), axis = 0).astype(np.int32)
...
def _fast_hist(label_true, label_pred, n_class):
    ...
    #print(np.unique(label_pred[mask]))
    #print(np.unique(label_true[mask]))
    #print ((n_class * label_true[mask].astype(int) + label_pred[mask]).max())
    #assert (np.unique(label_pred[mask]).max()<0)
    #assert ((n_class * label_true[mask].astype(int) + label_pred[mask]).max()<400)
    hist = np.bincount(
